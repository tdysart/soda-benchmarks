...
* [ll_to_verilog_legacy_tb.sh](ll_to_verilog_legacy_tb.sh) - Synthesizes `<input.ll>` with a bambu 2024 that has `--testbench-style=legacy` (branch `feature/legacy-xml-testbench` of `tdysart/PandA-bambu`): the same pure-Verilog testbench, with no llvm-cbe step. `testbench_to_xml.py --arch-xml --expected-from` writes its inputs, and the verilator-sst script above runs its output. Wired up in [mkinc/llvm_to_verilog_legacy_tb.mk](mkinc/llvm_to_verilog_legacy_tb.mk).
